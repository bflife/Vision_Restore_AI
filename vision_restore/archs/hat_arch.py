...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    # From PyTorch official master until it's in a stable release
    def norm_cdf(x):
    ...
    with torch.no_grad():
        # Values are generated by using a truncated normal distribution probability
        # density function (PDF).
        # as proposed by "Burr, I. W. (1942). Cumulative moment function. Journal of the American Statistical Association."
...
class HAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask):
        ...
        if self.shift_size > 0:
            ...
            attn_windows = self.attn(x_windows, rpi=rpi_sa, mask=None) # No mask needed if no shift? Actually mask is passed.
            # If no shift, mask is usually None?
            # Wait, original code passed attn_mask regardless.
            # But if shift_size=0, usually mask is None.
            # Let's stick to using the exact logic from HAB.forward logic above.
            # Re-reading check: if shift_size > 0: attn_mask = attn_mask else: attn_mask = None
            # Okay, I will correct that logic inside forward (it was split in my read code).
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):
        ...
        # Original: kv_windows = rearrange(kv_windows, 'b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch', nc=2, ch=c, owh=self.overlap_win_size, oww=self.overlap_win_size).contiguous()
        b, _, nw = kv_windows.shape
        kv_windows = kv_windows.view(b, 2, c, self.overlap_win_size * self.overlap_win_size, nw)
        kv_windows = kv_windows.permute(1, 0, 4, 3, 2).contiguous()
        kv_windows = kv_windows.view(2, b * nw, self.overlap_win_size * self.overlap_win_size, c)
...
class AttenBlocks(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, compress_ratio, squeeze_factor, conv_scale, overlap_ratio, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False):
        ...
        self.overlap_attn = OCAB(dim=dim, input_resolution=input_resolution, window_size=window_size, overlap_ratio=overlap_ratio, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, mlp_ratio=mlp_ratio, norm_layer=norm_layer)
...
class RHAG(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, compress_ratio, squeeze_factor, conv_scale, overlap_ratio, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, img_size=224, patch_size=4, resi_connection='1conv'):
        ...
        self.dim = dim
        self.input_resolution = input_resolution
        self.residual_group = AttenBlocks(dim=dim, input_resolution=input_resolution, depth=depth, num_heads=num_heads, window_size=window_size, compress_ratio=compress_ratio, squeeze_factor=squeeze_factor, conv_scale=conv_scale, overlap_ratio=overlap_ratio, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, downsample=downsample, use_checkpoint=use_checkpoint)
